...
战场背景图放进 `raw/bg/`。生图提示词见 [`PROMPTS.md`](PROMPTS.md)（12 角色姿势 + 背景场景）。
...
`raw/bg/*.png` → `public/backgrounds/<名字>.webp`（**2480×1400** cover 裁切，覆盖 Retina 物理像素）
+ `manifest.json`。管线会自动**压暗 28% + 去饱和**（不再加模糊）——生成原图时不必刻意做暗，
但建议：**16:9 构图、宽 ≥2480**（不足会被放大变软）、中部留干净站位区、避免高对比细碎纹理。

游戏战场的「背景」下拉框读 manifest 自动列出全部背景，无需改代码。
**中文显示名**：在 `packages/client/src/i18n.tsx` 的 `BG_NAME_ZH` 词典里加一条
`文件名: '中文名'`；没加的回退显示文件名（英文模式显示首字母大写的文件名）。
